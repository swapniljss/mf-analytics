          ...
          {logsLoading ? <Spinner /> : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {fileLogs?.map((log) => (
                    <tr key={log.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="table-cell"><span className="badge-blue">{log.module_name}</span></td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 text-xs max-w-[200px]">
                      ...
                      <td className="table-cell text-right text-green-600 dark:text-green-400">{log.row_count_inserted?.toLocaleString() || '—'}</td>
                      <td className="table-cell text-right text-red-500 dark:text-red-400">{log.row_count_rejected?.toLocaleString() || '—'}</td>
                      <td className="table-cell text-gray-500 dark:text-gray-400">{formatDate(log.created_at)}</td>
                    </tr>
                  ))}
                  {!fileLogs?.length && <tr><td colSpan={7} className="text-center py-8 text-gray-400 dark:text-gray-400 text-sm">No logs yet</td></tr>}
                </tbody>
              </table>
            </div>
          )}
          ...
          {jobsLoading ? <Spinner /> : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {jobs?.map((job) => (
                    <tr key={job.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="table-cell font-medium">{job.job_type}</td>
                      <td className="table-cell">
                        <Badge label={job.status || '—'} variant={statusBadgeVariant(job.status)} />
                      </td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 text-xs">{formatDate(job.started_at)}</td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 text-xs">{formatDate(job.completed_at)}</td>
                      <td className="table-cell text-red-500 dark:text-red-400 text-xs max-w-[200px]">
                        <span className="block truncate">{job.error_message || '—'}</span>
                      </td>
                    </tr>
                  ))}
                  {!jobs?.length && <tr><td colSpan={5} className="text-center py-8 text-gray-400 dark:text-gray-400 text-sm">No jobs yet</td></tr>}
                </tbody>
              </table>
            </div>
          )}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {issues?.map((issue: any) => (
                    <tr key={issue.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="table-cell font-medium">{issue.issue_type}</td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 text-xs">{issue.entity_key || '—'}</td>
                      <td className="table-cell">
                      ...
                      <td className="table-cell text-gray-500 dark:text-gray-400">{formatDate(issue.created_at)}</td>
                    </tr>
                  ))}
                  {!issues?.length && <tr><td colSpan={5} className="text-center py-8 text-gray-400 dark:text-gray-400 text-sm">No open issues</td></tr>}
                </tbody>
              </table>
            </div>
          )}
