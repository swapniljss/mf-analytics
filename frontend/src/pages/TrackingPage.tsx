      ...
      <div className="card">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <input
            ...
            className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 flex-1 min-w-[180px]"
          />
          <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {activeTab === 'error'
                  ? teData?.items.map((te) => (
                    <tr key={te.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="table-cell max-w-[240px]"><span className="block truncate font-medium">{te.scheme_name}</span></td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 max-w-[160px]"><span className="block truncate text-xs">{te.amc_name}</span></td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 max-w-[180px]"><span className="block truncate text-xs">{te.benchmark_name || '—'}</span></td>
                      <td className="table-cell text-right font-semibold text-orange-600 dark:text-orange-400">{formatPercent(te.tracking_error)}</td>
                      <td className="table-cell"><span className="badge-blue">{te.period_type || '—'}</span></td>
                      <td className="table-cell text-gray-500 dark:text-gray-400">{formatDate(te.as_of_date)}</td>
                    </tr>
                  ))
                  : tdData?.items.map((td) => (
                    <tr key={td.id} className="border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="table-cell max-w-[240px]"><span className="block truncate font-medium">{td.scheme_name}</span></td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 max-w-[160px]"><span className="block truncate text-xs">{td.amc_name}</span></td>
                      <td className="table-cell text-gray-500 dark:text-gray-400 max-w-[180px]"><span className="block truncate text-xs">{td.benchmark_name || '—'}</span></td>
                      <td className="table-cell text-right font-semibold text-purple-600 dark:text-purple-400">{formatPercent(td.tracking_difference)}</td>
                      <td className="table-cell text-gray-500 dark:text-gray-400">{formatDate(td.report_month)}</td>
                    </tr>
                  ))
                }
              </tbody>
